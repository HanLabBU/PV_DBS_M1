...
%% Loop through each region and condition
...
for f_region = fieldnames(region_matfiles)'
    ...
    for dbs =1:2
        ...
        for matfile =matfiles
            frs=[2:1:10 11:1:150 152:2:180];FS=round(1000./1.2);
            ...
            for tr=1:length(data.align.trial)  % trials
                if ~isempty(  data.align.trial{tr}) 
                    if mean(data.align.trial{tr}.spike_info375.spike_snr{1})>4 % Trial included if spike SNR above 4SNR
                        ...
                        allVm(:, tr_iter)=traceDBS;%voltsig;
                        [filt_sig]=Multi_func.filt_data(traceDBS,frs, FS); % filtering+Hilbert
...
% Old save path  
%savepath='\\engnas.bu.edu\research\eng_research_handata\eng_research_handata3\Pierre Fabris\PV Project\Figures_Eric\PLV_DBS_stim\'
pheight=150;
...
%% PLV for Spike-DBS train
for area=1:2
    for cond=1:2
        %M= [allPLVb{area,cond}] ;
        M1= [allPLVs{area,cond}] ;
        %  M2= [allPLVp{1,cond},allPLVp{2,cond}] ;
        figure('COlor','w','Position', [ 300 400 400 400],'Renderer', 'painters')
        yline(0);
        %fill_error_area2(frs, nanmean(M,2),nanstd(M,[],2)./sqrt(sum(~isnan((M(1,:))))  ) ,[ .5 .5 .5] )
        fill_error_area2(frs, nanmean(M1,2),nanstd(M1,[],2)./sqrt(sum(~isnan((M1(1,:))))  ) ,[ .5 .5 .5] )
        hold on;
        %fill_error_area2(frs, nanmean(M2,2),nanstd(M2,[],2)./sqrt(sum(isnan((M2(1,:))))  ) ,[ .5 .5 .5] )
        %plot(frs,nanmean(M,2),'b','Linewidth',1); hold on; 
        plot(frs,nanmean(M1,2),'g','Linewidth',1);
        %  plot(frs,nanmean(M2,2),'m','Linewidth',1)
        axis tight  ; 
        %ylim([ -0.05 0.5]);
        ax = gca;
        ...
        xlim([ 2 160]);
        title([DBS_reg{area}(3:end) ' ' DBS_cond{cond}(3:end)]);
        
        % set(gca,'Xscale','log')
        print(gcf, '-dpdf' , '-r300' ,'-painters', [ savepath 'PLV' f 'PLV_DBSStim_' DBS_reg{area}(3:end) '_area_high_' DBS_cond{cond}(3:end) '.pdf']);
        saveas(gcf, [ savepath 'PLV' f 'PLV_DBSStim_' DBS_reg{area}(3:end) '_' DBS_cond{cond}(3:end) '.png']);
    end
end
...
xlim([ 0.9 2.7]); %ylim([ -0.1 0.7])
   print(gcf, '-dpdf' , '-r300' ,'-painters', [ savepath 'Violin_gamma_' num2str(area) '_cond_' num2str(cond) '.pdf'])
