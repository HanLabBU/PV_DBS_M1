...
%% Loop through and calculate PLVs
allSmat=[];allPmat=[];allBmat=[];
        ...
        for matfile =matfiles
            ...
            for tr=1:length(data.align.trial)  % trials
                if ~isempty(  data.align.trial{tr}) 
                    ...
                    %    [wt, f] = get_power_spec(allVm(:, tr_iter),FS);
                    %     frs=f;
            ...
            if tr_iter   >0 
                ...
                if  mean(SNR) >3.6 % mean spike SNR for the neuron to be over 4SNR (can be changed)
                    ...
                    [ PLV_b, PLV_b2,matB]= Multi_func.spike_field_ppcDBS_Pierre(M,s,  shift_time);  % PLV estimatiom
                    clear s M;M=angle(wavD( StimTimsel,:,:));
                    allBmat=[allBmat,matB];
                    
                    for tr1=1:size(alls,2); 
                        s{tr1}=alls( StimTimsel,tr1)';
                    end
                    [ PLV_s, PLV_s2,matS]= Multi_func.spike_field_ppcDBS_Pierre(M,s,  shift_time); 
 allSmat=[allSmat,matS];
                    clear s M;M=angle(wavD(  PostTimsel,:,:));
                    for tr1=1:size(alls,2); 
                        s{tr1}=alls(  PostTimsel,tr1)';
                    end
                    
                    [ PLV_p, PLV_p2,matP]= Multi_func.spike_field_ppcDBS_Pierre(M,s,  shift_time); 
 allPmat=[allPmat,matP];
...
% Old save savepath
%savepath='\\engnas.bu.edu\research\eng_research_handata\EricLowet\DBS\PV_Pierre\PLV\'
pheight=130;

%%

% Boostrapping the phase-locked(non-adjusted) data
clear Sall Ball
nwin=30;
for fsel=1:size(frs,2)
    n1=size(allBmat,2);
    for id=1:100; % Is new phase-locking estimate for given frequency
        rsel=randperm(n1); % Selection of spikes
        BC=abs(nanmean(allBmat(:,rsel(1:nwin)),2) ); %nwin number of spikes to compute phase-locking, sub sample for each frequency
        Ball(fsel,id)=nanmean(BC(fsel)); % Use average phase-locking from spikes sampled to reassign the given frequency
    end
    n1=size(allSmat,2);
    for id=1:100;
        rsel=randperm(n1);
        SC=abs(nanmean(allSmat(:,rsel(1:nwin)),2) );
        Sall(fsel,id)=nanmean(SC(fsel));
    end
end


  figure('COlor','w','Position', [ 300 400 180 pheight],'Renderer', 'painters') 
    fill_error_area2(frs, nanmean(Ball,2) , nanstd(Ball,[],2)./sqrt(100)   ,[ .5 .5 .5] )
      fill_error_area2(frs, nanmean(Sall,2) , nanstd(Sall,[],2)./sqrt(100)   ,[ .5 .5 .5] )
  plot(frs, nanmean(Ball,2) ,'COlor',[57, 77, 161]/255,'Linewidth',1); hold on,
     plot(frs, nanmean(Sall,2) ,'COlor',[131, 195, 65]/255,'Linewidth',1)
   %set(gca,'Xscale','log')
    axis tight  ; ylim([ 0.1 0.3]); xlim([ 2 30])   
print(gcf, '-dpdf' , '-r300' ,'-painters', [ savepath 'Theta_reduct_PLOT_pop' num2str(switch_Source) '_' num2str(dbs) '.pdf'])

%%
fsel=frs>=3 & frs<=10 ; 
  BC=abs(nanmean(allBmat,2) );
  SC=abs(nanmean(allSmat,2) );
trueDIFF=mean(SC(fsel)-BC(fsel));


Mall=[allBmat,allSmat];
n1=size(allBmat,2);
n2=size(allSmat,2);
k1=size(Mall,2);
clear allstats
...
id
   rsel= randperm(k1);
   Mtest=Mall(:,rsel(1:n1));   Mtest1=Mall(:,rsel(n1+1:end));
  BC=abs(nanmean(  Mtest,2) ); SC=abs(nanmean(  Mtest1,2) );
allstats(id)=mean(SC(fsel)-BC(fsel));
end
trueDIFF
prctile(allstats,2.5);% 140, S, 40,S
%%%%%%%%%%%%%%
  BC=abs(nanmean(allBmat,2) );
  SC=abs(nanmean(allPmat,2) );
trueDIFF=mean(SC(fsel)-BC(fsel));
Mall=[allBmat,allPmat];n1=size(allBmat,2);n2=size(allPmat,2);k1=size(Mall,2);
clear allstats
for  id=1:1000
id
   rsel= randperm(k1);
...
trueDIFF
prctile(allstats,2.5) % 140, NS,40, NS
%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%
  BC=abs(nanmean(allSmat,2) );
  SC=abs(nanmean(allPmat,2) );
trueDIFF=mean(SC(fsel)-BC(fsel));
Mall=[allSmat,allPmat];n1=size(allSmat,2);n2=size(allPmat,2);k1=size(Mall,2);
clear allstats
for  id=1:1000
id
   rsel= randperm(k1);
...
trueDIFF
prctile(allstats,97.5) % 140, NS,40 S
%%%%%%%%%%%%%%%%%%


nwin=30;
fsel=frs>=3 & frs<=10 ; n1=size(allBmat,2);
clear Ball
for id=1:100;
    rsel=randperm(n1);
  BC=abs(nanmean(allBmat(:,rsel(1:nwin)),2) );
  Ball(id)=nanmean(BC(fsel));
end
n1=size(allSmat,2);
clear Sall
for id=1:100;
    rsel=randperm(n1);
  SC=abs(nanmean(allSmat(:,rsel(1:nwin)),2) );
  Sall(id)=nanmean(SC(fsel));
end
n1=size(allPmat,2);
clear Pall
for id=1:100;
    rsel=randperm(n1);
  PC=abs(nanmean(allPmat(:,rsel(1:nwin)),2) );
  Pall(id)=nanmean(PC(fsel));
end
% 140, NS
...
violinplot2(MM2,[ 2.3],'ViolinColor', [ 0.5 0.2 0.6; 0.9 0 0])
line([ 0.7 2.3], [ 0  0],'COlor', [ 1 1 1 ],'Linewidth',0.5);axis tight
xlim([ 0.9 2.6])
%print(gcf, '-dpdf' , '-r300' ,'-painters', [ savepath 'Violin_theta_pop' num2str(switch_Source) '_' num2str(dbs) '.pdf'])
...
%print(gcf, '-dpdf' , '-r300' ,'-painters', [ savepath 'THETA_PLV_across_area_' num2str(switch_Source) '_' num2str(cond) '.pdf'])
...
% df=60, 
 %0.0015
% 
%  [h,p,ci,stats] =ttest2(MM2,MM)
%  % Area
% sm=1;area=2;area2=2;cond2=1; cond=2
% for area=1:2
% 
%     M= [allPLVb{area,cond},allPLVb{area,cond2}] ;
%     M1= [allPLVs{area,cond},allPLVs{area,cond2}] ;
%    %   M2= [allPLVp{area,cond},allPLVp{area2,cond},allPLVp{area,cond},allPLVp{area2,cond}] ;
%     M(M==0)=NaN ;M1(M1==0)=NaN;
%     figure('COlor','w','Position', [ 300 400 180 pheight],'Renderer', 'painters') 
...
%    % fill_error_area2(frs, fastsmooth(nanmean(M2,2),sm,1,1),nanstd(M2,[],2)./sqrt(sum(isnan((M2(1,:))))  ) ,[ .5 .5 .5] )
%     plot(frs,fastsmooth(nanmean(M,2),sm,1,1),'b','Linewidth',1); hold on; plot(frs, fastsmooth(nanmean(M1,2),sm,1,1),'g','Linewidth',1)
%    % plot(frs, fastsmooth(nanmean(M2,2),sm,1,1),'m','Linewidth',1)
%     %  plot(frs,nanmean(M2,2),'m','Linewidth',1)
%     set(gca,'Xscale','log')
%     axis tight  ;% ylim([ -0.04 0.1]); 
%     xlim([ 2 30])   
%   print(gcf, '-dpdf' , '-r300' ,'-painters', [ savepath 'THETA_PLV_area_' num2str(switch_Source) '_' num2str(area) '.pdf'])
% end
% 
% 
%  sm=1;area=1;area2=2;cond2=2;% across areas
% for cond=1:2
%     M= [allPLVb{area,cond},allPLVb{area2,cond},] ;
%     M1= [allPLVs{area,cond},allPLVs{area2,cond}] ;
%    %   M2= [allPLVp{area,cond},allPLVp{area2,cond},allPLVp{area,cond},allPLVp{area2,cond}] ;
%     M(M==0)=NaN ;M1(M1==0)=NaN;
%     figure('COlor','w','Position', [ 300 400 180 pheight],'Renderer', 'painters') 
%     fill_error_area2(frs, fastsmooth(nanmean(M,2),sm,1,1),nanstd(M,[],2)./sqrt(sum(isnan((M(1,:))))  ) ,[ .5 .5 .5] )
...
% 
%  area=1
%  cond=1;cond1=cond; % per areas and cond
% M= [allPLVb{area,cond},allPLVb{area,cond},allPLVb{area,cond1},allPLVb{area,cond1}] ; M1= [allPLVs{area,cond},allPLVs{area,cond},allPLVs{area,cond1},allPLVs{area,cond1}] ;
%    M2= [allPLVp{area,cond},allPLVp{area,cond},allPLVp{area,cond1},allPLVp{area,cond1}] 
% fsel=frs>=3 & frs<= 9;   M(M==0)=NaN;M1(M1==0)=NaN;M2(M2==0)=NaN;
...
% xlim([ 0.9 2.6])
% print(gcf, '-dpdf' , '-r300' ,'-painters', [ savepath 'Violin_theta_' num2str(switch_Source) '_' num2str(cond) '_' num2str(area) '.pdf'])
% [h,p,ci,stats] =ttest2(MM,MM1)
% 
% 
 
 
 %% Calculate PLV for theta region
 figure('Position', [100 100 500 300], 'Renderer', 'painters');
 for region = 1:2
     for cond = 1:2
