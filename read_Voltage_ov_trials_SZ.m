...
start_trial = split(file_split{end}, '.');
start_trial = str2num(start_trial{1}(end-1:end));
...
for i=1:length(dcimgfile_names)
    if strcmp(dcimgfile_names{i}, ses) 
...
end%filname= [path1 'control_placefielkd_stim00014.dcimg'];
...
prenum=(n(10).name(1:3));
  ...
  %tr_sel=find(adc7>0);
  tr_sel=find(adc7>-3);  end;% check polarity of trigger
...
%   v= result.traces(result.trial_vec==ne,3);%./result.tracesB(result.trial_vec==ne)   ;
  v=v-fastsmooth(v,1400,1,1); 
    vS=double(zscore(v-fastsmooth(v,7,1,1))>2.3);vS(vS==0)=NaN;
    vS(1:50)=NaN;allV(:,ne)=v;  vS(end-50:end)=NaN;   v=zscore(v); v(1:50)=NaN;;
 v(end-50:end)=NaN;;
    allVS(:,ne)=vS;
