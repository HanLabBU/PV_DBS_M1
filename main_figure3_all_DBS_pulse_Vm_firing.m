...
for ind=[1:length(ses)]
    ...
    try
        ...
        lfp=[];  asA=[];
        clear aV allV8 allV40 allV140 allS alls1;
        ...
        for tr=1:size(alls1,2)
            try
                ...
                for ind=1:length(trigs)-1
                    if trigs(ind)-windS>0  & trigs(ind) +windS < size(alls1,1) & (trigs(ind+1)-trigs(ind))> 5...
                            & trigs(ind)<FS
                        M=[M,  alls1( trigs(ind)-windS:trigs(ind)+windS,tr)];
                    end
                    
                    if trigs(ind)-windS>0  & trigs(ind) +windS < size(alls1,1) & (trigs(ind+1)-trigs(ind))>5...
                            & trigs(ind)>FS  &trigs(ind)<FS*2
        ...
        if ~isempty(MS1)
            ...
            allmod_V(:,mj)=nanmean(VS1,2);
            allmod_V(:,mj)=allmod_V(:,mj)-nanmean(allmod_V(:,mj));
        end
