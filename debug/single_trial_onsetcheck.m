...
for i=1:length(matfiles)
    ...
    % Plotting individual trials with each new figure as a neuron representation
    figure('Position', [0 0 1000 1000]);
    tiledlayout(length(trial_idxs), 1,'TileSpacing', 'compact', 'Padding', 'compact');
    ax = {};
    for j = trial_idxs
        ...
        ax{end + 1} = nexttile;
        plot((cur_fov_frame_time(:, end))*1000, detrend_subVm);
        hold on;
        xline((cur_fov_stim_time(:, end))*1000, 'b');
    ...
    savefig(['Individual/' matfile '_indi.fig']);
    saveas(gcf, ['Individual/' matfile '_indi.png']);
