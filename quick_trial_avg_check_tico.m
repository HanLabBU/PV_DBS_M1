...
% Loop through each trial of the 'Extracted_traces' folder
ses = dir([path '*.mat']);
all_trials_mat = {ses.name};

trial_nums = length(all_trials_mat);

% Trials
all_subVm = [];
all_rasters = [];
for trial=1:trial_nums
    trial
    
    data = load([path all_trials_mat{trial}]);
    neuron = data.neuron;
    % Loop through all ROIs
    for roi=1:length(neuron)
        cur_trace = neuron{roi}.raw_trace;
        spike_info = spike_detect_SNR_sim3(cur_trace, 3.75, 4, 7);
        
        neuron{roi}.spike_info = spike_info;
    ...
    % TODO save the firing rates into the data structure
    % Maybe just the neuron structure
    save([path all_trials_mat{trial}], 'neuron', '-append');
end

% Just wanted to do the spike detection
return;
