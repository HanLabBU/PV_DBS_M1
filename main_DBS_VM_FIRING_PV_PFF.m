...
for stim_freq=[ 0  1]
    ...
    for ind=1:length(ses)
        ...
        % Load trace data for individ
        Cpath= ses(ind).name;
        load(Cpath)  %loading
        
        trial_numb=unique(result.trial_vec);
        
        % First trial contains less than 3500 frames and there are more than 2 trials
        if length(find(result.trial_vec==trial_numb(1) )) <3500 & length(trial_numb)>2
            ...
            for  ne=unique(result.trial_vec) % trials
                ...
                % Grab the phase angle of both 40Hz and 140Hz for every trace??
                Fn = FS/2;FB=[ 35 45];
                [B, A] = butter(2, [min(FB)/Fn max(FB)/Fn]);
